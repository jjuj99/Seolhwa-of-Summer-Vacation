...
function scene:create( event )
	...
	background.fill = {
		type = "image",
		filename = "image/background/수화산 중심_빈 4개의 꽃병.jpg"
	}
	...
	-- json에서 정보 읽기
	local Data = jsonParse("json/13.json")
	...
	local function nextScript( )
		...
		if (index > #Data) then
			BGM = audio.loadSound("sound/13. 퇴산_겨울 폭풍 소리와 윙윙거리는 바람 소리.mp3")
			audio.play(BGM, {channel=1, loops=-1})
			composer.gotoScene('scenario14', options)
			composer.removeScene('scenario13')
			return
		end
	...
	function set:tap( event )
 	...
 	function item:tap( event )
 		...
 		composer.showOverlay('items3', option)
 	end
