...
function scene:create( event )
	...
	background.fill = {
		type = "image",
		filename = "image/background/수화산정상.png"
	}
	...
	-- json에서 정보 읽기
	local Data = jsonParse("json/11.json")
	...
	local function nextScript( )
		...
		if (index > #Data) then
			...
			composer.gotoScene('scenario12', options)
			composer.removeScene('scenario11')
			return
		end

		if (Data[index].type == "dialogue2") then
	...
	function set:tap( event )
 	...
 	function item:tap( event )
 		...
 		composer.showOverlay('items2', option)
 	end
