...
function scene:create( event )
	...
	background.fill = {
		type = "image",
		filename = "image/background/마을수화산없음노을.png"
	}
	...
	-- json에서 정보 읽기
	local Data = jsonParse("json/15.json")
	...
	local function nextScript( )
		...
		if (index > #Data) then
			audio.stop()
			audio.dispose(BGM)
			BGM = audio.loadSound("sound/15. 스크롤_A new start.mp3")
			audio.play(BGM, {channel=1, loops=-1})
			composer.gotoScene('ending1', options)
			composer.removeScene('scenario15')
			return
		end
